...
<body id="page-top">
	...
	$(function () {
		...
		$('#interestSubmit').click(function () {
			var frmi = document.getElementById('frminter');
			frmi.action="interestpol/insert"
			frmi.submit();
		})
		
		$('#interestSearch').click(function () {
			var frmi = document.getElementById('frminter');
			frmi.action="interestpol/search"
			frmi.submit();
		})
	})
...
<hr>
									...
									<c:if test="${!empty interests}">
										...
										<c:if test = "${interest.getUserid() == userInfo.id }">
											<tr>
												...
												<td><a href = "interestpol/delete?no=${interest.getNo() }">삭제</a></td>
			...
			<div class="col-6">
				<div class="card mb-4">
					<div class="card-header">주변 환경 정보 조회</div>
					<div class="card-body">
						<form
							class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100"
						...
						<div class="order_details_iner"
							...
							<table class="table table-active text-left">
								...
								<tbody>
									<c:if test="${!empty pollutions}">
										<c:forEach var="pollution" items="${pollutions}">
											<tr>
												<td><a href="javascript:;"><p
															onclick='Print("${pollution}", "${pollution.getLat() }", "${pollution.getLng() }", "${pollution.getName() }")'>${pollution.getName() }</p></a></td>
												<td>${pollution.getAddress() }</td>
